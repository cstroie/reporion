...
| A6 | App shell = mockup Shell B (Reading room) layout + Shell C's royal blue / lime / amber palettes, user-selectable, royal blue default. Supersedes the Workbench choice (A5's chrome) | A report is read far more than it is navigated: one centred column and a dock keep the document first; the list column and tab strip moved to a drawer and the dock. The warm Reading-room palette was rejected on taste |
