...
| D36 | Accounts and grants live in `data/users/{username}.json`, disk-authoritative like pages (invariant 1). **No `user_grants` cache table in `index.sqlite`**: a principal's grants are already fully resolved in PHP before any query runs, so `Search\Query::visibilityClause($principal)` takes them as bound SQL parameters instead of joining a cache of them — `index:rebuild` never touches `data/users/` at all, because there is nothing about accounts in the index to lose. A namespace grant is a **prefix match**: a grant on `reports:mri` covers everything under `reports:mri:` | The account store can't be SQLite-only without breaking the project's first invariant. Binding grants as parameters is simpler than a join and sidesteps an entire rebuild-drift class of bug the earlier join-table sketch would have created |
...
| A6 | App shell = mockup Shell B (Reading room) layout without its floating dock + Shell C's royal blue / lime / amber palettes, user-selectable, royal blue default. Site-wide actions in the top nav; page actions in a page header with a page-local tab row (Report · Edit · History · Compare · Patient · Assistant). Supersedes the Workbench choice (A5's chrome) | A report is read far more than it is navigated: one centred column keeps the document first. A floating dock mixes site-wide and page-specific actions with no fixed home; splitting them puts each action next to what it acts on. The warm Reading-room palette was rejected on taste |
