...
| `/{ns}:` (and `/:` for the root namespace, `$ns === ''`) | SSR, not island | `Controller\NamespaceController`: sub-namespace cards (`Index\Sqlite::listSubnamespaces()` — for root, every top-level namespace in the tree), a plain pages table (`listNamespace()`), and `_index`/`_template` reserved-page cards + a "Namespace description" panel (reads the page named like the namespace — `reports:mri:mioveni` for `reports:mri:mioveni:`, 2026-09-26 — else the `_index` page, rendered through the same `findByPath()`/`Storage::read()` visibility-checked path as any other page) — all visibility-filtered before the query (invariant 6); a namespace with nothing visible to the caller 404s the same as one that does not exist (invariant 9), and an invisible or absent `_index`/`_template` just omits its card; with no description, a writer gets an "Add a description" link to `/new?path={ns}`. Not built: bulk select/move/tag/export/visibility and "recent activity" (no audit log yet). Rendered in the app shell (top nav only, no page header) for every caller; an anonymous one gets Sign in instead of the account menu — it does not switch to `layout-public.php` yet, only the listing predicate is enforced. The ☰ drawer, not a persistent tree sidebar, is the shell's namespace navigation |
...
| `/{path}/edit` | **SSR form + editor island** — `Controller\EditorController` renders a single raw-document textarea that saves by plain POST without JavaScript; `assets/js/editor.js` (`data-island="editor"`) enhances it with a **local draft**: a second after typing stops the document goes to IndexedDB with the revision it was made on (D25). **A revision is written only on Save** — the button or Ctrl+S, the form's own POST, which also handles conflicts (decided 2026-09-26: autosaving to the server wrote a revision every few seconds of typing). On reopening, a draft made on the current revision is *offered* (Restore / Discard); any other draft is stale and dropped — applying it silently used to put an old text back over a newer one. Leaving with unsaved changes asks first. A preview toggle renders the body with the vendored marked.js, configured by `assets/js/markdown-preview.js` (shared with the D17 conformance harness). Images by clipboard paste or file drag go to `POST /api/v1/media` and land at the cursor as `![name](media:…)` in a paragraph of their own; text pastes are untouched (D24). **Formatting toolbar (phase 10, 2026-09-26)**: heading (`##`/`###`, never `#` — D30), bold/italic (Ctrl+B/Ctrl+I), lists, table (tab-separated lines convert), code, link to a page (a picker over `GET /api/v1/search`), attach image (the same upload), insert prior study (the patient's other reports the caller can read — `Service\PatientStudies`, the timeline's lookup — as a link, and added to `priors`), insert template (`templates:{modality ns}:*`, its body via `GET /api/v1/pages/{path}?render=0`), copy the text without metadata. The two lists come in the island config; no new endpoint. The transforms are pure functions in `assets/js/editor-format.js` (tested in node from `tests/Render/EditorFormatTest`), applied with `execCommand('insertText')` so undo keeps them; everything they write is in the conformance corpus (`tests/fixtures/render/editor-toolbar.md`). No measurement macro (D18); snippets wait for D24 (TODO.md idea 9). Not built: AI rail. Rendered in the app shell with the page header (Edit tab active, shown only to a caller who can write here); Cancel sits in the save bar |
| `/new` | **SSR, not island** — `Controller\NewPageController`. **Under `reports:` — the guided new-report form (phase 7, built 2026-09-26)**, for the owner and anyone with an editor grant under `reports:` (`Service\NewReport`, `templates/new-report.php`, the mockup's `WikiCreate`): patient (one name field; optional CNP — checksum-validated, `Support\Cnp`, fills sex and birth year), exam (date, optional time, modality, site and device from Admin → Settings, regions), template (the caller's readable pages under `templates:{modality-ns}:`, body and exam fields only — `Service\Duplicates`), referrer, indication. One POST: `action=preview` recomputes the D1 path (`reports:{modality-ns}:{site}:{yymmdd}-{slug(name)}`, the modality → namespace map in Admin → Settings → Reports) and the next accession; `action=create` also refuses a path the caller cannot write (422), asks for an explicit `confirm_same_day` when the index already has a report for this patient on that date (strong or weak key, listing predicate — FORMATS §1), then allocates the accession (`Service\Accessions`, D20) and creates a private draft — **titled by the patient** (`title` and a first `#` heading are the name; the exam title goes to `exam_title`, which exports and public views use — D30 as amended 2026-09-26) and **with no template text** (the template gives metadata only: regions when none are ticked, protocol, `template`) — audited `page.create`, and opens the editor. **`?after={pid}`** (phase 9): a new exam for the patient of that report — the form prefilled with the patient, the same modality, site, device, regions and referrer, the previous `summary` as the indication, today's date and that report as the prior (a ticked box; `priors` is re-validated on submit, readable reports only). The pid, never the path, is in the URL; 404 for a report the caller cannot read or a page that is not one. The action is in a report's ⋯ menu and on the patient timeline ("New exam", from the newest readable report). Works without JavaScript; `assets/js/new-report.js` only previews the path and the CNP and narrows devices/templates. Elsewhere (`?ns=` outside `reports:`, `?mode=path` — "advanced: path and raw document") the plain colon-path field and raw document; `?from=` duplicates and `?path=` prefills as before |
...
| `/admin/*` | island | settings, tags, plugins, index — dense forms, rarely used, no print need. **Built as SSR forms** (no island yet): users, index & storage, trash, **maintenance** (`/admin/maintenance`, below) and **tags** (`/admin/tags`, `AdminTagsController` → `Service\Tags`, 2026-09-26): every tag with its page count; rename (`POST /admin/tags/rename { from, to }`) and merge (`POST /admin/tags/merge { from[], into }`) write a new revision of every *unsigned* page carrying it, audited `page.save` with reason `tag-rename`; signed reports keep their tags (D3). The mockup's groups, synonyms, ICD-10 codes and suggested merges have no data behind them and are not shown |
| `/admin/maintenance` | SSR | **Built 2026-09-26** (`AdminMaintenanceController` → `Service\Maintenance`): the maintenance commands of `bin/reporion` from the browser — `journal:replay`, `pages:check-frontmatter`, `index:verify`, `trash:purge` — a fixed list of tasks with typed options, run in-process (never by shelling out). Owner-only, 404 otherwise. `POST /admin/maintenance/{task}` with `mode=check\|apply` (apply needs `confirm=1`) stores the run report in `data/maintenance/runs/` and redirects to `GET /admin/maintenance?run={id}` (Post/Redirect/Get — a refresh never re-runs). `GET /admin/maintenance/runs/{id}.json` is the stored report, the same JSON `bin/reporion <task> --json` prints: `{task, mode, actor, options, started, finished, exit, summary, items: [{pid, rev, outcome, detail, data}], notes}`, pages by pid only. Apply runs, `index:rebuild` (CLI and Admin → Index) share one non-blocking lock (`data/maintenance/.lock`); busy answers "in progress". Runs are audited `maintenance.run`; the per-page writes keep their own audit lines. Long passes set `set_time_limit(300)` — FPM's `request_terminate_timeout` must allow it |
| `/admin/settings` | SSR | **Built 2026-09-26** (`AdminSettingsController` → `Service\InstanceSettings`): this instance's settings in `data/settings.yaml` (docs/FORMATS.md §3d) — site (name, tagline, public address, home page, time zone, icon), publishing and export switches, limits, sites and devices. Owner-only, 404 otherwise. `POST /admin/settings/{site\|publishing\|limits\|sites}` saves one section and redirects back (422 with the message for a bad value, nothing written); `POST /admin/settings/icon` takes the image as the request body (PNG/ICO/GIF/WebP ≤ 512 KB, no SVG). Audited `settings.change`. `GET /site-icon/{file}` (versioned, cached a year) and `GET /favicon.ico` serve the icon to everyone; every layout links it |
...
| `/export/{path}.{fmt}` | SSR | streams a file; `fmt` ∈ pdf, odt, md. **Built for pdf only** (`ExportController::pdf()`, dompdf via `Service\PdfExport`, D34): the print template byte for byte; a draft is refused with 409 unless `export.allow_draft_export`; anonymous only for a public page with `export.allow_public_export`; file name `{accession or pid}-rev{N}.pdf`, never the path (invariant 8); every export audited (`action: export`). **ODT built 2026-09-26** (`ExportController::odt()` → `Service\OdtExport`): the same print HTML through PHPWord's HTML import into an editable OpenDocument text, downloaded as `{accession or pid}-rev{N}.odt`, same access, draft and audit rules. PHPWord reads inline styles only and its ODT writer drops them inside table cells, so the few print.css rules that matter are inlined and turned into named styles; line breaks inside a table cell collapse. Written with PHPWord's bundled PclZip when PHP has no zip extension. Markdown not built **Reports and other pages** (2026-09-26, `Support\ReportPath`: under `reports:` with a `YYMMDD-name` last segment): a report prints with `templates/print/report.php` (letterhead, patient block, signature) and a draft report is refused as above; **any other page** — including a site's description page under `reports:` — prints with `templates/print/page.php` (site name, title, text, `rev N · date`, verification link) and exports as a draft too, since only reports are ever signed |
...
| `POST /pages/{path}/sign` | **built** (`PagesApiController::sign()`). `{ parafa? }` → 200 with the signed record, or `422 { error: { code: 'incomplete', fields: { missing: [...dotted field names] } } }` if a `required`/`required_for: ["sign"]` field is empty. D37: signing follows the write grant — whoever holds `editor`/`owner` on `path`'s namespace signs it as themselves, same 404-not-403 rule as every other write; idempotent per revision — signing an already-signed revision again is a no-op, not a second signature. Checks and audit live in `Service\Signing`, shared with the browser's **Sign** button (2026-09-26): `GET/POST /{path}/sign` (`Controller\SignController`), shown in the page header's tab row on a *draft report* the caller may write. The GET is a confirmation step naming the revision and the signer, listing any required field still empty (with a link to the editor instead of the button); the POST takes `base_rev` + optional `parafa` and signs only if `base_rev` is still the current revision — otherwise 409 and the form comes back naming the new one, so a signature always covers what was reviewed. Reports only (`Support\ReportPath`); anything else, or a caller without write access, is 404 |
...
| `DELETE /pages/{path}` | soft delete → `trash/`; requires `editor`/`owner` on `path`'s namespace. A page with pages under it (it shares its name with their namespace) is refused, 409 `has_children` — the browser's Delete says the same. `?purge=1` is **owner-only** (D3b: a stricter bar than ordinary delete) and audited |
...
- **Heading anchors** (2026-09-26): every heading gets `id` = its slug (`Support\Slug`, repeats numbered `-2`, `-3`, `section` when nothing is left) — what the table of contents links to — in both parsers; the preview's slug folds diacritics like the PHP one, and differs only for the few letters PHP transliterates further (ß, æ, ø), which Romanian text never has.

- **Links between pages** (decided 2026-09-26) are the one place the dialect takes an input besides the markdown: the mount point. The canonical form is the colon path, `[text](reports:mri:page)` (optionally `/`-prefixed, optionally `#fragment`); the importer's older `ns/page` form is recognised too, so imported and signed reports link correctly without a new revision (D3). Both resolve to `{basePath}/ns:page` — `Support\InternalLink` in PHP, the same rules in `assets/js/markdown-preview.js`, and the conformance test runs a fixture under a mounted base path. `media` and URI schemes (`tel:`, `mailto:`, …) are never pages. Print and export (`PrintView`, hence PDF and ODT) keep a page link's text and drop its address, which would carry a patient path (invariant 8). Body links are indexed as `links.kind = link`, which is what the page's backlinks panel lists — through the same visibility predicate as everything else.
...
POST /media?page={path}&name={file}  raw image body → 201 { sha256, ext, name, w, h, url, markdown }   (built)
GET  /media/{sha256}.{ext}           content-addressed, immutable, private long cache              (built)
```

**Media — built 2026-09-26** (`Controller\MediaController`, D27/D10). `POST /api/v1/media` takes the image itself as the request body (the editor sends the pasted or dropped `File` as it is — no multipart), PNG, JPEG, GIF or WebP by content sniffing (no SVG: it can carry script), up to `media.max_bytes` (default 8 MiB; PHP's `post_max_size` must allow it); 413 / 422 otherwise. Write access to `page`. The file is stored once in `data/media/{year}/{sha256}.{ext}` and attached to the page's `media.json`; the response's `markdown` is `![name](media:{sha256}.{ext})`, which both parsers resolve to `{basePath}/media/…` and print/PDF embed as a data URI (attached files only). Audited `media.attach`. `GET /media/{sha256}.{ext}` answers only a caller who can open (`pageAccessClause`) a page the file is attached to — indexed as `links.kind = media` — else 404; `Cache-Control: private, max-age=31536000, immutable`, `nosniff`. Publishing a page (D16) lists how many attached images become public with it.
...
|---|---|---|
| editor | the form's POST save, `/media`, `/templates`, `POST /sign`, `/ai/complete` (when enabled) | buffer, dirty flag, IndexedDB draft (with its base revision), marked.js preview |
...
> ⚠︎ **The editor is the one that must survive a dropped VPN (D12).** Typing goes to an IndexedDB draft keyed by page, carrying the revision it was made on; nothing is written to the server until the user saves. If the save cannot reach the server, the draft is still there — offered back when the editor is next opened on that same revision — and a save against a newer revision raises the conflict panel. This is the single most important piece of client-side engineering in the project — everything else can afford to fail by showing an error.
