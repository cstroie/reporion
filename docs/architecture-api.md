...
| `/{path}/history` | SSR | revision list + unified diff, both computed server-side. **Built**: `Controller\HistoryController`. Each row links a "vs current" diff (a plain `?from=&to=` link, works without JS); the mockup's `.wk-radio` multiselect "compare selected" is a small inline script that picks two revisions and navigates to the same `?from=&to=` URL — progressive enhancement, not an island. Carries the Workbench rail + tab strip + worklist sidebar + status bar (A5); the standalone "Back to page" button it used to have is gone — the tab strip's Report tab is the same link |
| `/{path}/compare?with=` | SSR | two reports side by side; the AI delta arrives by API afterwards. **Built differently**: `Controller\CompareController` serves `/{path}/compare?from=N&to=M` — two *revisions of the same page* (default previous→current), not two reports; no `?with=`, no AI delta. Report-vs-report compare is still open (docs/roadmap.md) |
| `/{path}/print` | SSR | print stylesheet, letterhead, QR. Must work with JS disabled |
| `/patient/{key}` | SSR | timeline; a document about a person, not an app. **Built at `/{path}/timeline` instead** (`Controller\TimelineController`): the patient is resolved from that page's indexed `patient_key`, so the URL carries a page path, not a key. Which of the two is canonical is an open decision (docs/roadmap.md, phase 3) |
| `/search?q=` | SSR + island | first result page rendered so the URL is shareable; facets then live |
| `/{path}/edit` | **SSR form + editor island** — `Controller\EditorController` renders a single raw-document textarea that saves by plain POST without JavaScript; `assets/js/editor.js` (`data-island="editor"`) enhances it with 3 s debounced autosave via `PUT /api/v1/pages/{path}`, an IndexedDB draft (D25) and the 409 conflict panel. A marked.js preview toggle is an inline script in the template. Not built: AI rail, template panel, `/media` paste. Carries the Workbench rail + tab strip + worklist sidebar + status bar (A5); the standalone Cancel link next to Save is gone — the tab strip's Report tab is the same navigation (an unsaved edit is discarded either way, same as Cancel always did) |
| `/new` | **SSR, not island** — `Controller\NewPageController`. The mockup's segmented `reports:{modality}:{site}:{yymmdd}-{name}` builder is built as plain inputs; a small inline script assembles them into the submitted `path` (the server only ever sees `path`). Not built: live index validation of the path, the template picker actually applying a template |
...
> **A6 supersedes A5's shell choice (2026-09-25):** the app shell is now the Reading room layout with the Workbench palettes (`design/README.md` §"Chosen direction", `docs/DECISIONS.md` A6). A5's principle stands unchanged — chrome is a reskin, never a routing change: every top-nav item, page tab and drawer link is a plain `<a>` to a real route. The paragraph below describes the Workbench chrome as built, which phase 1 of `docs/roadmap.md` replaces.
