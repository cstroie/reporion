...
| `/{path}/history` | SSR | revision list + unified diff, both computed server-side. **Built**: `Controller\HistoryController`. Diff is "vs current" per row (a plain `?from=&to=` link, no JS), not an arbitrary-pair compare — the mockup's radio-multiselect "compare selected" isn't built (see below). Carries the Workbench rail + tab strip + worklist sidebar (A5); the standalone "Back to page" button it used to have is gone — the tab strip's Report tab is the same link |
...
| `/{path}/edit` | **SSR, not island** — `Controller\EditorController`, a single-textarea raw-document form (no autosave/preview/AI rail/JS conflict UI — see below and docs/BUILD_LOG.md). Carries the Workbench rail + tab strip + worklist sidebar (A5); the standalone Cancel link next to Save is gone — the tab strip's Report tab is the same navigation (an unsaved edit is discarded either way, same as Cancel always did) |
